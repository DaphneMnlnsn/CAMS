...
switch ($action) {
    case 'archive':
        $stmt = $conn->prepare("UPDATE section SET archived = 1 WHERE section_id = ?");
        break;
    case 'restore':
        $stmt = $conn->prepare("UPDATE section SET archived = 0 WHERE section_id = ?");
        break;
    case 'delete':
...
if ($stmt->execute()) {
    $desc = ucfirst($action) . "d section ID $id";
