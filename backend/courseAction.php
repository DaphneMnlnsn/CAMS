...
switch ($action) {
    case 'archive':
        $stmt = $conn->prepare("UPDATE course SET archived = 1 WHERE course_id = ?");
        break;
    case 'restore':
        $stmt = $conn->prepare("UPDATE course SET archived = 0 WHERE course_id = ?");
        break;
    case 'delete':
...
if ($stmt->execute()) {
    $desc = ucfirst($action) . "d course ID $id";
